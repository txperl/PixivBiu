...
- **Env-var convention**: prefix `PIXIVBIU_`, underscores map to dots. Examples:
  ...
  - `PIXIVBIU_LOG_LANGUAGE=ja` → `log.language`
...
- **Startup banner + lifecycle logs are localized** via `internal/i18n` (`log.language`: `auto` = sniff `LANG`→`LC_ALL`→`LC_MESSAGES`, else explicit `en`/`zh-CN`/`ja`; default `auto`). `cmd/server/main.go` builds one translator (`i18n.New(i18n.Resolve(cfg.Log.Language))`) and threads it through `printBanner` + the ~6 lifecycle messages (server starting / shutdown signal / restart draining / re-exec / server stopped / drain-timeout). **Everything else stays English** — every structured slog field key, the per-request `http.request` line, and reload-failure diagnostics. `i18n.T(key, args…)` falls back missing-locale→EN and missing-key→the key string itself; printf-style verbs interpolate `args`. That translator is built once at startup (and the banner only prints at boot), so `log.language` is **restart-required**: a runtime PATCH persists but surfaces in `pending_restart` until `POST /config/restart` re-execs and rebuilds it — it is **not** wired into the `OnReload` hooks.
...
Per-field `cfg:` tags also drive `x-cfg-restart-required` / `x-cfg-advanced` / `x-cfg-internal` / `x-cfg-sensitive` hints used by the settings UI. The **restart-required** set (changes only take effect after `POST /config/restart`) is: `server.host`, `server.port`, `server.timeouts.{read,write,shutdown}`, `log.format`, `log.language`, `pixiv.bypass_sni`, `pixiv.state_file`, `download.max_concurrent`, `download.store_file`, `inbox.buffer_size`. The **advanced** set (de-prioritised in the settings UI — sorted below everyday fields within a section, and whole sections made up entirely of this tier sink to the bottom + hide behind the "advanced" toggle) is: `log.level`, `log.format`, `log.language`; `internal` fields are folded into this same tier (and additionally rendered read-only). The **internal** (program-only — editable only by hand-editing `settings.json`; PATCH/keyed-reset rejected, `reset {all:true}` preserved, UI read-only) set is: `server.*`, `pixiv.state_file`, `download.{referer,store_file}`, `inbox.*`. Everything else hot-reloads and is PATCH-able.
