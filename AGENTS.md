...
- **i18n**: [Paraglide JS](https://inlang.com/m/gerre34r/library-inlang-paraglideJs) — compile-time, tree-shakeable message functions. Source/config/generated all consolidated under `src/i18n/` (barrel `index.ts`; `messages/` + `project.inlang/` + `generated/` (gitignored) + `react/{locale-provider,use-messages}`). Locales: `en` (baseLocale + fallback), `zh-CN`, `ja`. Paraglide strategy: `localStorage` → `baseLocale` (no `preferredLanguage`). Locale resolution is two-stage: (1) on mount, `LocaleProvider` resolves `auto` against `navigator.languages` *only* when Paraglide's localStorage cache is empty (first visit) — otherwise the cache wins, preserving the user's last explicit choice; (2) once auth lands, `<LocaleSync>` (mounted inside `AuthProvider`) fetches `GET /api/v1/config` and applies the persisted `app.language`. The fetch is auth-gated, so the unauthenticated login page relies on stage 1. Prefix matching is hand-rolled — `zh*` → `zh-CN`, `ja*` → `ja`, `en*` → `en` — because Paraglide's `extractLocaleFromNavigator` only matches exact configured locales (and `zh` is not in our list, so it would miss `zh-Hans-CN`, `zh-TW`, etc.). `applyLanguage(configured)` fires synchronously after a `PATCH /config` that touched `app.language`, so a save switches the UI without a reload. Inside React render paths, read messages via `const m = useMessages()` — it subscribes to `LocaleContext`, so switching language triggers a rerender without remounting the subtree. **Do not** `import { m }` from `@/i18n/generated/messages` directly inside components: those importers don't subscribe, so language switches won't refresh them. Message keys are flat **snake_case with a domain prefix** (`common_`/`nav_`/`login_`/`downloads_`/`search_`/`filter_`/`ranking_`/`user_`/`home_`/`status_`/`error_`/`settings_`/`cfg_`) and must exist in all three JSONs with the same `{param}` shape — see `src/i18n/messages/README.md`. For a lookup keyed by a runtime value, use an **explicit static map** (`{ key: () => m.key() }`), never `m[dynamicKey]()` (breaks tree-shaking + types). Two such resolvers exist: `lib/api/error-message.ts::useApiErrorMessage()` maps the 8 backend error codes → `m.error_*` (falls back to `error.message`), and `features/settings/i18n.ts::{useFieldText,useSectionTitle}` map each config `field.key` → `m.cfg_*` and section id → `m.settings_section_*`, **falling back to the backend's Chinese `field.description`/title when a key is absent** — the `/config/schema` text is the safety net and is itself never localized. The backend logs in English regardless of `app.language`; it just persists the field and surfaces it through `GET /api/v1/config` — locale resolution lives entirely in the frontend. Module-level constants that hold UI text must move inside the component (or a `useMemo`), or they freeze the locale at import time.
...
- `app.language` — frontend UI language: `auto` (frontend resolves via `navigator.language`) | `en` | `zh-CN` | `ja` (default `auto`). The backend stores and serves the raw value via `GET /api/v1/config`; resolution happens entirely client-side, so this is **not** restart-required — a PATCH switches the UI immediately. Backend logs/banner are always English regardless.
