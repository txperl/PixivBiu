...
- For pages that show illust lists: pipe raw illusts through `useFilteredIllusts(...)` from `@/features/filter`, feed `filtered` into the grid and into `useQuickActionPanel`'s `allIllustIds`, and register `useFilterPanel({ specialFilters, specialFiltersActiveCount, onResetSpecialFilters, totalBefore, totalAfter })`. Pass `null` (or omit the call) to opt out — panel falls back to its EmptyState. `specialFiltersActiveCount > 0` lights the red indicator dot on the bar icon and renders `(n)` next to the section header. `onResetSpecialFilters` renders a "重置" button next to the section header (disabled when count is 0); pass `null` when the page has no `specialFilters`. Compose `specialFilters` content as `<FilterRow label inactive>` rows from `@/features/filter` so unset fields render muted.
