...
│   │   ├── users.yaml            #     /users/*
│   │   └── search.yaml           #     /search/*
│   ├── openapi.bundled.json      #   ⚠️ Generated by `make bundle` — never commit / never edit
│   └── cfg.yaml                  #   oapi-codegen configuration
├── cmd/
│   ├── bundle/
│   │   └── main.go               # Resolves $refs across api/* into api/openapi.bundled.json (pre-codegen step)
│   └── server/
│       └── main.go               # Entry point: load config → init logger → run HTTP server → graceful shutdown
├── internal/
│   ├── api/
│   │   ├── gen.go                #   //go:generate directive (bundle + oapi-codegen)
...
| `make bundle` | Resolve all `$ref`s in `api/openapi.yaml` into `api/openapi.bundled.json` |
| `make gen`    | `bundle` + regenerate `internal/api/server.gen.go` |
...
Because oapi-codegen doesn't resolve external `$ref`s across files, `make gen` first runs `cmd/bundle/main.go` to produce `api/openapi.bundled.json` with all refs internalized, then hands that bundled file to oapi-codegen.
...
2. **Run `make gen`** — it bundles then regenerates `internal/api/server.gen.go`. The generated file contains:
...
- ⚠️ **DO NOT edit `api/openapi.bundled.json`.** It is regenerated by `make bundle` from `openapi.yaml` + `paths/*.yaml` and is gitignored.
