...
│   ├── i18n/                     # Locale resolver for app.language (Resolve only — backend logs are English)
...
- **Backend logs are English**, always — including the startup banner, the ~6 lifecycle messages (server starting / shutdown signal / restart draining / re-exec / server stopped / drain-timeout), structured slog fields, the per-request `http.request` line, and reload-failure diagnostics. `internal/i18n` no longer carries a translation catalog; it only resolves `app.language` (`auto` = sniff `LANG`→`LC_ALL`→`LC_MESSAGES`, else explicit `en`/`zh-CN`/`ja`; default `auto`) into a concrete `Locale` once at boot. That resolved locale is stored on `APIHandler` and surfaced via `GET /api/v1/i18n` so the frontend UI can mirror it without sniffing `navigator.language`. `app.language` is **restart-required**: the locale is pinned at boot, so a runtime PATCH persists but surfaces in `pending_restart` until `POST /config/restart` re-execs — it is **not** wired into the `OnReload` hooks.
...
- `app.language` — frontend UI language: `auto` (backend sniffs `LANG`/`LC_*`) | `en` | `zh-CN` | `ja` (default `auto`). The frontend reads the resolved value via `GET /api/v1/i18n` and mirrors it (never sniffs `navigator.language` on its own). Backend logs/banner are always in English regardless of this setting. **restart-required** (the locale is resolved once at boot, so a PATCH surfaces in `pending_restart`).
