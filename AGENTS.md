...
├── .github/workflows/            # CI (push/PR); Release+Docker on v* (GoReleaser + signed R2 feed / GHCR multi-arch); Desktop on desktop-v* (Electron installers → own R2 feed)
...
- **Version & update trust anchor (link-time stamping).** Three values are injected via `-ldflags -X`. `main.version` (git-describe locally, the tag in releases; the Docker build passes it as the `VERSION` build-arg — `docker` if unset) is printed in the boot banner. The self-updater's **trust anchor** — `main.updateFeedURL` (the CDN feed base) and `main.updateTrustedKeysRaw` (the trusted minisign public key(s), comma-separated) — is stamped the same way. It is **empty in source**, so a plain `go build` / `make build` fails closed (no trusted key → no update is ever offered); releases inject it from the `UPDATE_FEED_BASE` / `UPDATE_PUBLIC_KEYS` repo **variables** (GoReleaser and the Docker build both read them), so a fork self-publishes updates without editing Go source. `cmd/server/main.go::parseTrustedKeys` tokenizes the stamped key list; the release workflow validates and normalizes the anchor before publishing. Setup, rotation, and the fork flow live in [docs/RELEASE.md](docs/RELEASE.md).
...
- **Updates owned by `electron-updater`.** Whole-app updates (signed + notarized on macOS) replace the core's self-updater in desktop builds — hence `PIXIVBIU_APP_UPDATE_ENABLED=false`. `electron-updater` reads a **desktop-only feed** on Cloudflare R2 (`generic` provider, `<DESKTOP_FEED_BASE>/desktop`, e.g. `https://dl.biu.tls.moe/desktop`) — a separate path from the core self-updater's signed manifest, and fully isolated from the core `v*` GitHub Releases so it never mistakes a core release for a desktop one. Integrity rides electron-updater's native `latest*.yml` sha512 + the macOS code signature (no minisign here). The SPA's existing About / release-notes UI is reused: `UpdateProvider` (`frontend/src/features/system/update-provider.tsx`) is desktop-aware and drives `status`/`checkNow`/`apply` off the `window.pixivbiu.updates` bridge instead of `/system/update*`.
