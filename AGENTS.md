...
├── .github/workflows/            # CI (push/PR) + Release & Docker & Desktop (v* tag → GoReleaser + signed R2 feed / GHCR multi-arch / Electron installers)
...
- **Build.** `.github/workflows/desktop.yml` (on a `v*` tag, additive to GoReleaser/Docker) builds per-OS on a matrix: each runner builds the SPA + core (macOS = universal `lipo` binary named `pixivbiu`), stages it into `desktop/resources/`, then `electron-builder --publish` to the same GitHub Release (the feed electron-updater reads). Locally: `make desktop-dev` / `make desktop-dist`. macOS signing/notarization needs the `CSC_*` + `APPLE_*` secrets; Windows/Linux ship unsigned in v1. Full guide in [desktop/README.md](desktop/README.md).
