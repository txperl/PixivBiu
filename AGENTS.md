...
- **i18n**: [Paraglide JS](https://inlang.com/m/gerre34r/library-inlang-paraglideJs) — compile-time, tree-shakeable message functions. Source/config/generated all consolidated under `src/i18n/` (barrel `index.ts`; `messages/` + `project.inlang/` + `generated/` (gitignored) + `react/{locale-provider,use-messages}`). Locales: `en` (baseLocale + fallback), `zh-CN`, `ja`. Paraglide strategy: `localStorage` → `baseLocale` (no `preferredLanguage` — the frontend never sniffs `navigator.language`; instead the `LocaleProvider` fetches `GET /api/v1/i18n` on mount and `setLocale()`s to whatever the backend resolved, so `app.language` is the single source of truth and `localStorage` is just a first-paint cache). Inside React render paths, read messages via `const m = useMessages()` — it subscribes to `LocaleContext`, so switching language triggers a rerender without remounting the subtree. **Do not** `import { m }` from `@/i18n/generated/messages` directly inside components: those importers don't subscribe, so language switches won't refresh them. Message keys are flat **snake_case with a domain prefix** (`common_`/`nav_`/`login_`/`downloads_`/`search_`/`filter_`/`ranking_`/`user_`/`home_`/`status_`/`error_`/`settings_`/`cfg_`) and must exist in all three JSONs with the same `{param}` shape — see `src/i18n/messages/README.md`. For a lookup keyed by a runtime value, use an **explicit static map** (`{ key: () => m.key() }`), never `m[dynamicKey]()` (breaks tree-shaking + types). Two such resolvers exist: `lib/api/error-message.ts::useApiErrorMessage()` maps the 8 backend error codes → `m.error_*` (falls back to `error.message`), and `features/settings/i18n.ts::{useFieldText,useSectionTitle}` map each config `field.key` → `m.cfg_*` and section id → `m.settings_section_*`, **falling back to the backend's Chinese `field.description`/title when a key is absent** — the `/config/schema` text is the safety net and is itself never localized. The backend has its own tiny catalog for the startup banner + lifecycle logs (see Logging) — driven by the same `app.language` setting, so the two sides never disagree. Module-level constants that hold UI text must move inside the component (or a `useMemo`), or they freeze the locale at import time.
...
│   ├── i18n/                     # Tiny en/zh-CN/ja catalog for the startup banner + lifecycle logs (T + Resolve)
...
- **Startup banner + lifecycle logs are localized** via `internal/i18n` (`app.language`: `auto` = sniff `LANG`→`LC_ALL`→`LC_MESSAGES`, else explicit `en`/`zh-CN`/`ja`; default `auto`). `cmd/server/main.go` builds one translator (`i18n.New(i18n.Resolve(cfg.App.Language))`) and threads it through `printBanner` + the ~6 lifecycle messages (server starting / shutdown signal / restart draining / re-exec / server stopped / drain-timeout). **Everything else stays English** — every structured slog field key, the per-request `http.request` line, and reload-failure diagnostics. `i18n.T(key, args…)` falls back missing-locale→EN and missing-key→the key string itself; printf-style verbs interpolate `args`. That translator is built once at startup (and the banner only prints at boot), so `app.language` is **restart-required**: a runtime PATCH persists but surfaces in `pending_restart` until `POST /config/restart` re-execs and rebuilds it — it is **not** wired into the `OnReload` hooks. The frontend mirrors the resolved value via `GET /api/v1/i18n` so its UI language always matches the backend (it never sniffs `navigator.language` itself).
...
- `app.language` — application UI + startup banner + lifecycle-log language: `auto` (follow `LANG`/`LC_*`) | `en` | `zh-CN` | `ja` (default `auto`). The frontend reads the resolved value via `GET /api/v1/i18n` and mirrors it (never sniffs `navigator.language` on its own). Only localizes the banner + lifecycle lines on the backend; structured slog fields stay English. **restart-required** (the translator is built once at boot, so a PATCH surfaces in `pending_restart`).
...
| GET | `/i18n` | `getI18n` | Returns `{configured, locale}`. `configured` is `app.language` verbatim (may be `auto`); `locale` is the concrete locale the running process resolved (`en`/`zh-CN`/`ja`). Frontend reads this on boot to mirror the backend's UI language. |
