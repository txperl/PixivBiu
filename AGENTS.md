...
The current backend covers **auth + read-only browsing + bookmark/follow + download + SSE event stream** (all backed by [github.com/txperl/pixivgo](https://github.com/txperl/pixivgo)), plus **version reporting & one-click self-update from GitHub Releases (minisign-verified on official builds)**, and a **same-origin image proxy with on-disk cache**. Search cache and SauceNAO remain intentionally deferred.
...
- **i18n**: [Paraglide JS](https://inlang.com/m/gerre34r/library-inlang-paraglideJs) — compile-time, tree-shakeable message functions. Source + config + generated output consolidated under `src/i18n/` (barrel `index.ts`; `messages/` + `project.inlang/` + `generated/` (gitignored) + `react/{locale-provider,use-messages}`). Locales: `en` (baseLocale + fallback), `zh-CN`, `zh-TW`, `ja`. The rules below are load-bearing:
...
│   ├── update/                   # GitHub Releases checker + Apply (download → verify SHA-256 vs signed checksums.txt → selfupdate binary swap → restart)
...
├── desktop/                      # Electron desktop shell — thin wrapper around the single binary (see Build & Release → Desktop)
│   ├── src/                      #   main.ts (lifecycle) · core-process.ts (sidecar) · oauth-window.ts (auto-login) · preload.ts (window.pixivbiu bridge) · updater.ts
│   ├── electron-builder.yml      #   packaging / signing / publish; build/entitlements.mac.plist for hardened runtime
│   └── package.json              #   electron + electron-builder + electron-updater (npm; dist/ + node_modules gitignored)
├── .github/workflows/            # CI (push/PR); Release+Docker on v* (GoReleaser + signed checksums / GHCR multi-arch); Desktop on desktop-v* (Electron installers → PixivBiu-Desktop releases)
...
- **Version & update trust anchor (link-time stamping).** Two values are injected via `-ldflags -X`. `main.version` (git-describe locally, the tag in releases; the Docker build passes it as the `VERSION` build-arg — `docker` if unset) is printed in the boot banner. The self-updater's **trust anchor** — `main.updateTrustedKeysRaw` (the trusted minisign public key(s), comma-separated, accepted as signers of each release's `checksums.txt`) — is stamped the same way. It is **empty in source**, so a plain `go build` / `make build` produces a build that verifies updates by HTTPS + SHA-256 only (no signature requirement — a fork's updater works with zero key setup); releases inject it from the `UPDATE_PUBLIC_KEYS` repo **variable** (GoReleaser and the Docker build both read it), making official builds **signature-enforcing**: a release without a valid `checksums.txt.minisig` is refused, and a stamped-but-malformed key still fails closed. The update source (`repoOwner`/`repoName`, `cmd/server/main.go`) is a pair of consts — a rebranding fork edits those. `cmd/server/main.go::parseTrustedKeys` tokenizes the stamped key list; the release workflow validates the keys and their pairing with the signing secret before publishing. Setup, rotation, and the fork flow live in [docs/RELEASE.md](docs/RELEASE.md).
- **Release binary + self-update resolution.** The release binary is named `pixivbiu` (`.goreleaser.yaml` `binary:`), lowercase like a Unix tool; the distributable archive keeps the brand-cased `PixivBiu_<ver>_<os>_<arch>` name (`project_name`), and the Windows `.exe` ships without an embedded icon/version-info resource (the Electron desktop app owns the polished Windows packaging — see [Desktop](#desktop-electron)). The in-app updater (`internal/update/apply.go::extractBinary`) locates the executable inside a downloaded archive by **matching the running binary's own base name** (`os.Executable`) against the archive's executable members (the execute-bit files in a tar.gz, the `.exe` files in a zip) — so a rename can't break self-update, and an archive bundling several executables still resolves to the app binary. With no name match it falls back to the lone executable, and refuses (rather than guessing) when several are present but none matches.
...
- **Release** (`.github/workflows/release.yml`, on `v*` tag): [GoReleaser](https://goreleaser.com) (`.goreleaser.yaml`) builds the frontend (via `make build-web`), cross-compiles linux/macOS/windows × amd64/arm64 (`CGO_ENABLED=0`), and publishes archives + SHA-256 checksums + a grouped changelog (its commit range is chosen per channel — a stable aggregates its whole pre-release cycle) as a **GitHub Release — the source the in-app updater reads**. GoReleaser's `signs` block **minisign-signs `checksums.txt`** (`checksums.txt.minisig` ships as a release asset); a verified checksums file transitively authenticates every archive, and the signing key never leaves CI (the `MINISIGN_SECRET_KEY` secret). The workflow validates the trust anchor **before** GoReleaser publishes (key shape + secret/public pairing probe) and re-verifies the real signature after. **Tags must be strict semver** (`v3.0.0`, prerelease `v3.0.0-beta.1`) — legacy `v2.x.ya/b` suffixes are rejected by GoReleaser. Dry-run with `goreleaser release --snapshot --clean --skip=sign` (or set `MINISIGN_SECRET_KEY_FILE` to a local key to exercise signing). Channels, the signing setup, the required secrets/keys, and the full tag→audience mapping live in [docs/RELEASE.md](docs/RELEASE.md).
...
`desktop/` is a **thin Electron shell** around the **unchanged** single Go binary — the core stays the self-contained release; the desktop is an additive package on top. The shell (`desktop/src/main.ts`) spawns the `pixivbiu` binary as a child "sidecar" (`core-process.ts`), waits for `/api/v1/health`, then loads the embedded SPA from `http://127.0.0.1:<port>`. No Go code changes — the shell only sets pre-existing flags/env. The **core never computes OS paths; the shell owns OS placement** and passes each dir in:

| Data | Location | Passed via |
|------|----------|-----------|
| settings / auth state / download index | `userData/usr/` (outside the bundle, survives updates) | `PIXIVBIU_DATA_DIR` |
| image cache (purgeable, ≤2 GiB) | OS cache dir (`~/Library/Caches/PixivBiu`, `%LOCALAPPDATA%\PixivBiu\Cache`, `~/.cache/PixivBiu`) | `PIXIVBIU_CACHE_DIR` (→ `runtimepath.CacheRoot`) |
| logs (rotating) | OS logs dir (`app.getPath('logs')/pixivbiu.log`) | `PIXIVBIU_LOG_FILE` (→ `log.file`) |
| downloads | `~/Downloads/PixivBiu` (first-run seed only, then user-editable) | `download.output_dir` seeded into `usr/settings.json` |

- **Automated login (the reason a desktop build exists).** Instead of the DevTools copy/paste in the [OAuth (PKCE) login flow](#token-state), a Chromium window opens Pixiv's hosted login and intercepts the `…/auth/pixiv/callback?code=…` redirect itself (`desktop/src/oauth-window.ts`), feeding the code into the **existing** `POST /auth/oauth/exchange`. The SPA feature-detects the shell via `window.pixivbiu` (`frontend/src/lib/desktop.ts`) and swaps its login step (`pages/login/index.tsx`, `login-panel.tsx`) for a one-click "Sign in with Pixiv"; the refresh-token paste stays as a fallback. Real Chromium renders Pixiv's reCAPTCHA / 2FA natively.
- **Updates owned by `electron-updater`.** Whole-app updates (signed + notarized on macOS) replace the core's self-updater in desktop builds — hence `PIXIVBIU_APP_UPDATE_ENABLED=false`. `electron-updater` reads the **dedicated artifacts-only repo** [`txperl/PixivBiu-Desktop`](https://github.com/txperl/PixivBiu-Desktop) (`github` provider, plain `vX.Y.Z` tags) — it cannot share this repo: the provider watches the newest release in its target repo, and the core's own `v*` releases (no `latest*.yml`) would shadow the desktop ones. Integrity rides electron-updater's native `latest*.yml` sha512 + the macOS code signature (no minisign here). The SPA's existing About / release-notes UI is reused: `UpdateProvider` (`frontend/src/features/system/update-provider.tsx`) is desktop-aware and drives `status`/`checkNow`/`apply` off the `window.pixivbiu.updates` bridge instead of `/system/update*`.
- **First-run download seed.** `core-process.ts::seedFirstRunDefaults` writes `download.output_dir` = the OS Downloads folder into `usr/settings.json` **on first run only** — pre-boot, writing the file directly (the on-disk override layer, `internal/config/store.go`) because the config PATCH is auth-gated and first run is pre-login. Stays user-editable in Settings.
- **Two independent release trains.** The desktop app ships on its **own** cadence, decoupled from the core, so a shell-only change never forces a core release and a core-only change never forces a desktop one (no "phantom" updates on either side — see [docs/RELEASE.md → Desktop release train](docs/RELEASE.md#desktop-release-train)). The core `v*` train (GoReleaser + Docker + CLI self-updater) is unchanged; **frontend changes ride the core train** (the SPA is `go:embed`-ed into the core, never shipped standalone). The desktop `desktop-v*` train (`.github/workflows/desktop.yml`) does **not** rebuild anything: `scripts/stage-core.sh` downloads the core release pinned in **`desktop/.core-version`** and stages the binary per-arch into `desktop/resources/<arch>` (macOS stages **both** `darwin_amd64` and `darwin_arm64` slices — no lipo; electron-builder embeds the matching slice via `extraResources: from: resources/${arch}/`), then `electron-builder --publish always` publishes the installers + `latest*.yml` (+ stable `PixivBiu-latest-*` download aliases via `gh release upload`) into a **pre-created draft release on `PixivBiu-Desktop`** that a final job flips live only once all three platforms' update metadata is present (draft-then-publish, so updaters and `/releases/latest` links never see a half-built release; cross-repo writes ride the `DESKTOP_RELEASES_TOKEN` fine-grained PAT). macOS ships a **per-arch split** (separate arm64 + x64 dmgs, not a universal binary — a universal build would double the bundled ~85 MB core); Windows is x64 NSIS; Linux is x64 AppImage/deb/rpm. Bump `desktop/.core-version` (+ cut a new `desktop-v*` tag) to ship a newer core to desktop users (Docker-Desktop-style pinned bundle). Locally: `make desktop-dev` builds the core from source; `make desktop-dist` packages the host arch; `make desktop-fetch-core` reproduces CI's pinned-download path. macOS signing/notarization needs the `MAC_CSC_*` + `APPLE_*` secrets; Windows signs via Azure Trusted Signing when the `WIN_AZURE_*`/`AZURE_*` vars+secrets are set (else unsigned); Linux is unsigned. Full guide in [desktop/README.md](desktop/README.md); release/secrets reference in [docs/RELEASE.md](docs/RELEASE.md#desktop-release-train).
...
- `GET /system/version` and `GET /system/update` (cached check result) are **open** (no auth) so the About panel renders pre-login; the mutating checks (`POST /system/update/check`, `.../apply`) require auth. `POST /system/update/apply` additionally requires the SPA's `X-PixivBiu-App` request header as a **CSRF guard** (a cross-origin page can't add it without a CORS preflight the server never grants → 403) and is **single-flighted** (a second concurrent apply → 409); it downloads/verifies/swaps synchronously (the check/apply writes opt out of the server write deadline), flushes `202 {status:"updating"}`, then `restart()`s. Dev / `go run` builds report `is_dev:true` and refuse apply. The check lists releases via the **GitHub API** (unauthenticated; `internal/update/github.go`); `Apply` verifies the download's SHA-256 against the release's `checksums.txt`, whose **minisign signature (`checksums.txt.minisig`) is verified first on signature-enforcing builds** (trusted keys stamped at link time) — an unsigned or badly-signed release is refused (`bad_request`/400), while unstamped fork/dev builds verify by HTTPS + checksum alone. GitHub reachability/download failures surface as `upstream_error`/502 with an **update-specific** message (not the Pixiv-outage copy). `release_notes` is **server-normalized for display** — `internal/update/checker.go::{aggregateNotes,sanitizeReleaseBody}` strip each release body's commit SHA, `(@author)`, and leading `## Changelog`, and an update spanning several versions is stitched newest-first under per-version `## <tag>` headings (frontend maps `h2`→version, `h3`→GoReleaser group); the SPA's `ReleaseNotesDialog` only renders it, while "View on GitHub" still points at the raw release (the dual-published GitHub Release). The `X-PixivBiu-App` header is sent on every request by the shared api client (`lib/api/client.ts`); keep its name in sync with `appRequestHeader` (`handler_system.go`).
...
- Docker images (releases are single self-contained binaries — see [Build & Release](#build--release); CI + tag-driven GitHub Releases with minisign-signed checksums are implemented)
