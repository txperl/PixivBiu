...
- For pages that show illust lists: pipe raw illusts through `useFilteredIllusts(...)` from `@/features/filter`, feed `filtered` into the grid and into `useQuickActionPanel`'s `allIllustIds`, and register `useFilterPanel({ specialFilters, specialFiltersActive, onResetSpecialFilters, totalBefore, totalAfter })`. Pass `null` (or omit the call) to opt out — panel falls back to its EmptyState. `specialFiltersActive` lights the red indicator dot on the bar icon. `onResetSpecialFilters` renders a "重置" button next to the section header (disabled when `!specialFiltersActive`); pass `null` when the page has no `specialFilters`.
