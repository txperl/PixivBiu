...
All endpoints are mounted under `/api/v1`. Success responses return the resource directly (clean REST); errors return `{code, kind, message, fields?, upstream?, request_id?}` with an HTTP status code (see "Error classification" below for the field semantics).
...
| POST | `/auth/connectivity` | `checkConnectivity` | Pre-login Pixiv reachability probe (no auth). Optional body `{proxy?}` tests a candidate proxy; a working one is persisted to `pixiv.proxy` (pre-login only) so the imminent OAuth uses it. `reachable:false` is a normal 200, not an error. |
