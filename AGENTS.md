...
- **Auto-open browser.** `app.open_browser` (default `false`, `restart=true`) opens the web UI in the default browser at boot via the `internal/browser` launcher. Per-run override precedence: `-open` / `-open=false` CLI flag (highest) > `PIXIVBIU_APP_OPEN_BROWSER` > config. The flag is the only one of the three surfaces *not* in the reflected schema (it's a `flag.Visit` override in `cmd/server/main.go`), so it's documented here rather than discoverable via `/config/schema`. No-op on headless Linux; a future Electron shell sets the env to `false` so it doesn't double-open alongside its own window.
...
- `internal/pixiv/service.go` runs a background refresh loop that calls `client.Auth()` whenever the access token has < 5 min left, then overwrites the file. If that loop ever misses a window (sleep/wake, a transient failure), authenticated calls self-heal instead of forcing a restart: the `pixiv.Call` / `pixiv.Exec` wrappers (`internal/pixiv/call.go`) catch a 401, refresh once (single-flighted via `refreshMu`), and retry against a pinned same-identity client `Clone`. All ~16 authenticated call sites route through `Call` / `Exec`.
