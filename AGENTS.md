...
- **Two independent release trains.** The desktop app ships on its **own** cadence, decoupled from the core, so a shell-only change never forces a core release and a core-only change never forces a desktop one (no "phantom" updates on either side — see [docs/RELEASE.md → Desktop release train](docs/RELEASE.md#desktop-release-train)). The core `v*` train (GoReleaser + R2 + Docker + CLI self-updater) is unchanged; **frontend changes ride the core train** (the SPA is `go:embed`-ed into the core, never shipped standalone). The desktop `desktop-v*` train (`.github/workflows/desktop.yml`) does **not** rebuild anything: `scripts/stage-core.sh` downloads the core release pinned in **`desktop/.core-version`** (macOS prefers the `darwin_all` universal archive from GoReleaser's `universal_binaries`, else lipos the per-arch archives for older pins), stages the binary into `desktop/resources/`, then `electron-builder --publish never` and uploads the installers + `latest*.yml` (+ stable `PixivBiu-latest-*` download aliases) to the desktop R2 feed — desktop installers are **not** on GitHub Releases. Bump `desktop/.core-version` (+ cut a new `desktop-v*` tag) to ship a newer core to desktop users (Docker-Desktop-style pinned bundle). Locally: `make desktop-dev` builds the core from source; `make desktop-fetch-core` reproduces CI's pinned-download path. macOS signing/notarization needs the `MAC_CSC_*` + `APPLE_*` secrets; Windows/Linux ship unsigned in v1. Full guide in [desktop/README.md](desktop/README.md); release/secrets reference in [docs/RELEASE.md](docs/RELEASE.md#desktop-release-train).
