...
macOS keeps the invisible 44px top drag band without adding a toolbar or page padding. `desktop.css` limits automatic `no-drag` exceptions to controls inside `app-drag`, semantic headers/forms/navigation/toolbars/tab lists, and explicit `data-app-controls` groups. Mark a custom action row or settings form with `data-app-controls`; mark a custom interactive widget with `data-app-no-drag` (or `app-no-drag`). The group marker itself does not disable dragging in its empty space. Do not put these markers around artwork feeds or cards: ordinary content remains undeclared and clickable below the drag band, while artwork passing underneath the band must not change window dragging. Electron's native region calculation can include clipped, scrolled `no-drag` boxes; global declarations or explicit `app-region: none` resets reintroduce invisible holes. Shared portal surfaces, scrollbars and resize handles have macOS-only exceptions. These rules are inactive on Windows/browser/Linux and in fullscreen.
