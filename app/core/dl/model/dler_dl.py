...
class DlDler(Dler):
    def __init__(self, url, folder="./downloads/", name=None, dlArgs=Dler.TEMP_dlArgs, dlCacheDir=None, dlRetryMax=2,
        ...
        ifile.mkdir(self._dlCacheDir)
        ifile.mkdir(self._dlSaveDir)
    ...
    def clear_cache(self, isAllCache=False):
        if isAllCache:
            ifile.clearDIR(self._dlCacheDir)
        else:
            ifile.rm([x[0] for x in self._dlCacheBlockArr])
